...
T0 = 50; h = 1; 
yhatMA = zeros(T-h-T0+1,1); %% ARMA(3,1) forecasts
%theta = [phi 1, phi 2, phi 3; mu, psi]
f = @(theta) loglike_ARMA21(theta,y0,y(1:T0));
thetahat = fminsearch(f,[.5;.5;0.5;0;5]);
for t = T0:T-h
yt = y(1:t);
%% find the MLE
...
X = [[y0(m);y(1:t-1)] [y0(m-1:end);y(1:t-2)] [y0(m-2:end);y(1:t-3)] ones(t,1)];
uhat = H\(yt - X*[thetahat(1) thetahat(2) thetahat(3) thetahat(4)]');
%% store the forecasts
yhatMA(t-T0+1,:) = thetahat(4) + thetahat(1)*y(t)+ thetahat(2)*y(t-1)++ thetahat(3)*y(t-2) + thetahat(5)*uhat(end);
end
