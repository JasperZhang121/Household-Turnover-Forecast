...
load 'AUSRetail.csv' ;
y = AUSRetail(:,1);
AusRetailHoltFunction([0.7 0.3 0.5]);%MSFE for aplha = 0.7, beta = 0.3, gamma = 0.5
[parametersstar MSFEstar] = fminsearch(@AusRetailHoltFunction, [0.5 0.5 0.5])
toc

function MSFE = AusRetailHoltFunction(parameters)  %assumes y is already the variable of interest
load 'AUSRetail.csv' ;
y = AUSRetail(:,1); T = length(y);
T0 = 50; h = 1; s = 12;
