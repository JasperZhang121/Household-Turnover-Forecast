...
for t = T0:T-h
    yt = y(h:t);
    zt = [ones(t-h+1,1) [y0(m);y(1:t-h)] [y0(m-1:end);y(1:t-h-1)] [y0(m-2:end);y(1:t-h-2)] [y0(m-3:end);y(1:t-h-3)]];
    betahat2=(zt'*zt)\(zt'*yt);
%% store the forecasts
yhatIAR(t-T0+1,:) = [1 y(t) y(t-1) y(t-2) y(t-3)]*betahat2;
end
ytph = y(T0+h:end); % observed y {t+h}
MSFE_IAR4 = mean((ytph-yhatIAR).^2);